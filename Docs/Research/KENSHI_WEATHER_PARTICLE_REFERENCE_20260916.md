...
Phases A-D are implemented. What follows records what actually shipped and
where this plan was wrong.

### Ogre ABI: confirmed, not assumed

Every symbol the bridge needs is present in the export table of the shipped
Redux `OgreMain.dll` (Ogre 1.10, 32-bit, MSVC12/MSVCP120):

| Symbol | Used for |
|---|---|
| `StringInterface::setParameter` | generic set, for emitters and affectors both |
| `StringInterface::getParameter` | generic get |
| `StringInterface::getParameters` | parameter-name enumeration |
| `ParticleEmitter::getType` | emitter type name |
| `ParticleAffector::getType` | affector type name |
| `ParticleSystem::getNumAffectors` | affector count |
| `ParticleSystem::getAffector` | affector by index |

Two points the plan left open:

- `setParameter` and `getParameter` are virtual, but neither `ParticleEmitter`
  nor `ParticleAffector` overrides them - the base walks the `ParamDictionary`
  Ogre built for the concrete type. Calling the exported `StringInterface`
  implementation is therefore correct dispatch, not a vtable bypass. No vtable
  index is assumed anywhere.
- `getParameter` returns `Ogre::String` by value, so it is an sret call. The
  ABI notes are in `src/Ogre/OgreStringInterfaceShim.h`.

### Phase A and B: shipped

Nine Lua functions, in `src/Game/Environment.cpp` and registered in
`src/luaexport.cpp`:

```text
exu.GetParticleEmitterType            exu.GetParticleSystemAffectorCount
exu.GetParticleEmitterParameterNames  exu.GetParticleAffectorType
exu.GetParticleEmitterParameter       exu.GetParticleAffectorParameterNames
exu.SetParticleEmitterParameter       exu.GetParticleAffectorParameter
                                      exu.SetParticleAffectorParameter
```

`GetParticleEmitterParameterNames` / `GetParticleAffectorParameterNames` were
not in the plan. They were added because the plan repeatedly says to verify
parameter spellings at runtime before treating them as stable, and without an
enumerator the only way to do that is to guess a name and see whether
`setParameter` returns false.

The Lua-to-Ogre value conversion lives in `src/Ogre/OgreParameterValue.h`,
which deliberately has no Windows, Ogre or Lua dependency so it can be
compiled and tested on a host with none of them present.

### Phase C: shipped, with a smaller scope than planned

Shipped in `Workshop/`, which is a mod folder and therefore a resource
location the engine already scans:

- `exu_weather.particle` - `EXU/WeatherRainLight`, `EXU/WeatherRainHeavy`,
  `EXU/WeatherDustLocal`, `EXU/WeatherHaboobWall`, `EXU/WeatherMist`
- `exu_weather.material` - four materials inheriting the stock
  `BZSprite/AlphaBlend` render state through `EXU_FX/OgreParticleAlphaBlend`
- `exu_ogre_particle.program` plus SM4/GLSL sources - a narrow vertex-colour
  adapter for Ogre-generated billboards
- four placeholder billboard textures, generated by
  `tools/generate_weather_textures.py`

The material keeps the stock blend/depth/fragment behavior, while the EXU
vertex program avoids the stock SM4 BGRA correction. That correction is needed
for native BZR packed sprite vertices but is a second red/blue swap for Ogre
ParticleFX vertices, which have already gone through `convertColourValue`.
OpenShim's Enhanced scheme fallback can select the valid base technique; EXU
does not duplicate that global policy and depends on no other mod.

The textures are procedural and original, so there is nothing to license. They
are placeholders: good enough to profile and tune against, not final art.
`--check` in the generator compares decoded pixels rather than file bytes,
because PNG encoding is not stable across Pillow and zlib versions.
...
interpolation, eased wind, deterministic emission pulsing, fog and light
matching, baseline capture and restore. It has no scheduling and no random
weather, which stay with whoever owns the campaign.

Selecting the `clear` profile explicitly destroys active systems and restores
the captured baseline. This is intentionally tested as weather -> clear ->
later update so a cleared profile cannot strand storm fog or reapply it.

The module stem is exactly 11 characters on purpose. On a Workshop install
Redux's fallback asset searcher truncates module names to 11 characters, so a
longer name makes `require` fail with a misleading "no asset" error.

### Where this plan was wrong

The plan treats baseline weather as unbuilt. It is not: **Campaign Reimagined
already ships a mature weather stack** - `cr_weather.particle` (12 templates
covering dust, dust devil, snow, ash, spore, rain and splash) plus roughly 3700
lines of controller across `CRWeather.lua`, `CRWeatherPresets.lua` and
`CRMarsWeather.lua`, already driving EXU's particle API in production.

Two consequences:

1. Phase C was scoped down. Authoring EXU copies of rain and dust that CR has
   already tuned would have produced a worse duplicate of shipping content, and
   put campaign-flavoured art in the shared runtime repository. The templates
   here are the generic baseline a mission gets with no campaign installed. The
   haboob wall and the mist layer are the two effects neither CR nor the
   earlier `weather_particles.particle` had.
2. The real value of phases A and B is larger than the plan claims. CR's
   templates already declare `ColourFader`, `ColourInterpolator`, `Scaler`,
   `Rotator` and `DirectionRandomiser` affectors, and **none of them were
   reachable from Lua before this change**. Every one of those is now live-
   tunable on content that already ships.
