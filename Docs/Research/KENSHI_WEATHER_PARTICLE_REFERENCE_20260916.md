...
- `exu_weather.material` - four materials inheriting stock
  `BZSprite/AlphaBlend` from `sprites.material`
- four placeholder billboard textures, generated by
  `tools/generate_weather_textures.py`

Inheriting the stock sprite parent rather than declaring passes here means
there is no material scheme for a renderer to miss and fall back to black on,
and EXU depends on no other mod.
